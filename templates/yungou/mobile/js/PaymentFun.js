       ...
       // var v = 1;
        ...
        $("li", m).each(function(y) {
            ...
            if (y == 0) {//y=0表示没有支付列表的时候
                ...
                z.click(function() {
                    v = y;//当v>0时表示在支付列表中选择不支付方式
                    r = z.text();
                    z.children("i").attr("class", "z-bank-Roundsel");
                    z.siblings().each(function() {
                        $(this).children("i").attr("class", "z-bank-Round")
                    });
                    h.removeClass("z-pay-grayC").html('<s class="z-arrow"></s>选择<b class="z-mlr">' + r + "</b>支付" + ((g > 0 || w > 0) ? "剩余": "") + '<em class="orange">' + e + ".00</em>元")
                    checkpay='bank'
                    //banktype=r;
                if(r=='建设银行'){
                  banktype='CCB-WAP';
        ...
        var u = function() {
            ...
            if (w + g >= x) {
                ...
                if (e > 0) {//error v=5 v == 5为新加的//当v>0时表示在支付列表中选择了支付方式
                 //   if (v == 1 || v == 2 || v == 3|| v == 5) {
					 if(v >0){
                           location.href = Gobal.Webpath+"/mobile/cart/paysubmit/"+checkpay+"/"+banktype+"/"+x+"/"+t+"/"+submitcode
                    }
					else
					{
						alert("请您选择支付方式！");
