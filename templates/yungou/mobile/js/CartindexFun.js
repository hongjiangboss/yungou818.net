$(function() {
    ...
    var b = function() {
        ...
        var d = function() {
			...
			var last = Math.floor(parseInt(z.next().next().next().val()));
            ...
            var x = function() {
                ...
                if (y != "" && s != y) {
                    ...
                    if (w.test(y)) {
						...
						max_num = (last > max_num) ? max_num : last;
                        if (u <= max_num) {
                            q.val(y)
                        } else {
                            u = max_num;
        ...
        var p = function(r, u) {
            ...
            if (r > 100) {
                if (q.length == 0) {
                    var s = $('<div class="z-Cart-tips">欢迎光临！祝您购物愉快，好运连连！</div>');
                    t.prepend(s)
                }
            } else {
                q.remove()
        ...
        var j = function(q, t, r, pr) {
			g.children("a").unbind('click').css({"background":"#ccc none repeat scroll 0 0","border": "1px solid #ccc"});
            var s = function(w) {
				g.children("a").click(function() {
					location.href = Gobal.Webpath+"/mobile/cart/pay"      //付款页面
				}).css({"background":"#f0414e none repeat scroll 0 0","border": "1px solid #ef6000"});;
                if (w.code == 1) {
                    var v = $(window).width();
        ...
        var k = function(w, v) {
			...
			var last = Math.floor(parseInt(v.next().next().next().val()));
            var q = v.next().next();
            var pr = v.attr("price");
