				  ...
				  //alert(I.count)
                    if (I.code == 0) {
                        ...
                        for (var D = 0; D < C; D++) {
                            ...
                            var z = Math.ceil(G);
                            E += "<ul onclick=\"location.href='"+Gobal.Webpath+"/mobile/mobile/item/" + F.id + '\'"><li class="mBuyRecordL">';
                            E += '<img src2="'+Gobal.imgpath+'/uploads/' + F.thumb + '" src="' + Gobal.LoadPic + '"></li>';
                            E += '<li class="mBuyRecordR">(第' + F.qishu + "期)" + F.title + "";
                            if (F.codeState == 3) {
                                E += '<p class="mValue">价值：￥' + F.money + '</p><span>获得者：<a style="color: #9d1875" href="'+Gobal.Webpath+'/mobile/mobile/userindex/' + F.q_uid + '">' + F.q_user + '</a><br>幸运码：<em class="orange">' + F.q_user_code + "</em></span>"
                            } else {
