...
function send_mobile_shop_code($mobile=null,$uid=null,$code=null){
		...
		if(!$template){
			$template = array();
			$content =  "你在"._cfg("web_name")."购买的商品已揭晓,幸运码是:".$code;
		}	
		if(empty($template['value'])){
			$content =  "你在"._cfg("web_name")."购买的商品已揭晓,幸运码是:".$code;
		}else{
...
function send_email_code($email=null,$username=null,$uid=null,$code=null,$shoptitle=null){
	...
	if(!$template){
			$template = array();
			$template['value'] =  "恭喜您：{$username},你在". _cfg("web_name")."购买的商品{$shoptitle}已揭晓,揭晓码是:".$code;
	}else{	
	...
	$title = "恭喜您!!! 您在". _cfg("web_name")."购买的商品揭晓了!!!";		
	return _sendemail($email,'',$title,$template['value']);
}

function send_weixin_code($openid=null,$username=null,$uid=null,$code=null,$shoptitle=null,$TM=null,$qishu=null)
      {
	   $db=System::load_sys_class('model');
	   $imp=array('touser' => $openid,
		 'template_id' => "u5TkZ75Y2sg7ujC_1_2713Y-dFgDBwQe5oybqmmID_Q",
		 'url' => "http://m.yungou818.com/?/mobile/home/address/",
		 "topcolor"=>"red",
		 'data' => array(
		 'first' => array('value' =>"亲爱的：{$username}，你好，恭喜你获得（第".$qishu."期）{$shoptitle}商品",'color'=>"#737373"),
		  'tradeDateTime' => array('value' => date("y-m-d h:i:s",time()),'color'=>"#737373"),
		  'orderType' => array('value' => "云购商品订单",'color'=>"#737373"),
		  'customerInfo' =>array('value' => $TM,'color'=>"#FF0000"),
		  'orderItemName' => array('value' =>"客服热线:  400-1588-136  (8",'color'=>"#737373"),
		  'orderItemData' => array('value' =>"30-21:00) ",'color'=>"#737373"),
		  'remark'   => array('value' => "感谢对一元圆梦的支持，祝你好运!点击详情查看订单并填写收货地址", 'color'=>"#737373")
		  ));
	
		  return  _sendweixin($imp);
}
