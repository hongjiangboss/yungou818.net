...
HTML;
	...
	public function goods_list(){
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_shoplist` WHERE goods_type=0 and $list_where");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		$page->config($total,$num,$pagenum,"0");
		$shoplist=$this->db->GetPage("SELECT * FROM `@#_shoplist` WHERE goods_type=0 and $list_where",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));

		include $this->tpl(ROUTE_M,'shop.lists');
	}
//商品列表
	public function goods_list_l(){
		$this->ment=array(
						array("lists","商品管理",ROUTE_M.'/'.ROUTE_C."/goods_list"),
						array("add","添加商品",ROUTE_M.'/'.ROUTE_C."/goods_add"),
						array("renqi","人气商品",ROUTE_M.'/'.ROUTE_C."/goods_list/renqi"),
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_shoplist` WHERE goods_type=1 and $list_where");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		$page->config($total,$num,$pagenum,"0");
		$shoplist=$this->db->GetPage("SELECT * FROM `@#_shoplist` WHERE goods_type=1 and $list_where",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));

		include $this->tpl(ROUTE_M,'shop.lists');
	}
	//商品列表
	public function jf_goods_list(){
		$this->ment=array(
						array("lists","商品管理",ROUTE_M.'/'.ROUTE_C."/jf_goods_list"),
						array("add","添加商品",ROUTE_M.'/'.ROUTE_C."/jf_goods_add"),
						array("renqi","人气商品",ROUTE_M.'/'.ROUTE_C."/jf_goods_list/renqi"),
						array("danjia","单价倒序",ROUTE_M.'/'.ROUTE_C."/jf_goods_list/danjia"),
						array("money","商品价格倒序",ROUTE_M.'/'.ROUTE_C."/jf_goods_list/money"),
		);
	    $cateid=$this->segment(4);
		...
		if($cateid){
			...
			if($cateid=='qishu'){
				$list_where = "1 order by `qishu` DESC";
				$this->ment[4][1]="期数正序";
				$this->ment[4][2]=ROUTE_M.'/'.ROUTE_C."/jf_goods_list/qishuasc";
			}
			if($cateid=='qishuasc'){
				$list_where = "1 order by `qishu` ASC";
				$this->ment[4][1]="期数倒序";
				$this->ment[4][2]=ROUTE_M.'/'.ROUTE_C."/jf_goods_list/qishu";
			}
			if($cateid=='danjia'){
				$list_where = "1 order by `yunjiage` DESC";
				$this->ment[5][1]="单价正序";
				$this->ment[5][2]=ROUTE_M.'/'.ROUTE_C."/jf_goods_list/danjiaasc";
			}
			if($cateid=='danjiaasc'){
				$list_where = "1 order by `yunjiage` ASC";
				$this->ment[5][1]="单价倒序";
				$this->ment[5][2]=ROUTE_M.'/'.ROUTE_C."/jf_goods_list/danjia";
			}
			if($cateid=='money'){
				$list_where = "1 order by `money` DESC";
				$this->ment[6][1]="商品价格正序";
				$this->ment[6][2]=ROUTE_M.'/'.ROUTE_C."/jf_goods_list/moneyasc";
			}
			if($cateid=='moneyasc'){
				$list_where = "1 order by `money` ASC";
				$this->ment[6][1]="商品价格倒序";
				$this->ment[6][2]=ROUTE_M.'/'.ROUTE_C."/jf_goods_list/money";
			}
			if($cateid==''){
				$list_where = "`q_uid` is null  order by `id` DESC";
			}
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_jf_shoplist` WHERE $list_where");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		$page->config($total,$num,$pagenum,"0");
		$shoplist=$this->db->GetPage("SELECT * FROM `@#_jf_shoplist` WHERE $list_where",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));

		include $this->tpl(ROUTE_M,'shop.jf_lists');
	}
	...
	/* 单个商品的购买详细 */
	public function jf_goods_go_one(){
		$gid = intval($this->segment(4));
		$ginfo = $this->db->GetOne("select * from `@#_jf_shoplist` where `id` = '$gid' limit 1");
		if(!$ginfo)_message("没有找到这个商品");

		$total=$this->db->GetCount("select * from `@#_member_go_jf_record` where `shopid` = '$gid' order by `id` DESC");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		$page->config($total,20,$pagenum,"0");
		$go_list = $this->db->GetPage("select * from `@#_member_go_jf_record` where `shopid` = '$gid' order by `id` DESC",array("num"=>20,"page"=>$pagenum,"type"=>1,"cache"=>0));
		include $this->tpl(ROUTE_M,'shop.jf_go_list');
	}
	/* 手动揭晓	*/
	...
	//商品回收站
	public function jf_goods_del_list(){
		$this->ment=array(
			array("lists","返回商品列表",ROUTE_M.'/'.ROUTE_C."/jf_goods_list"),
			array("add","添加商品",ROUTE_M.'/'.ROUTE_C."/jf_goods_add"),
		);

		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_jf_shoplist_del` WHERE 1");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		$page->config($total,$num,$pagenum,"0");
		$shoplist=$this->db->GetPage("SELECT * FROM `@#_jf_shoplist_del` WHERE 1",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));


		include $this->tpl(ROUTE_M,'shop.jf_del');
	}
	...
	public function goods_edit(){
		...
		if(isset($_POST['dosubmit'])){
			...
			$is_share = !empty($_POST['is_share']) ? intval($_POST['is_share']) : 0;
			...
			$yuanjia = intval($_POST['yuanjia']) ? floatval($_POST['yuanjia']) : 0;
			$pv = intval($_POST['pv']) ? intval($_POST['pv']) : 1;
			$ispre = intval($_POST['ispre']);
			...
			$sql="UPDATE `@#_shoplist` SET `cateid` = '$cateid',
										   ...
										   `newpos` = '$newpos' ,
											`posthumb` = '$posthumb' ,
										   `ex_y`='".htmlspecialchars($_POST['ex_y'])."',
										   `pos` = '$goods_key_pos',
										   `is_share` = '$is_share',
	...
	//编辑商品
	public function jf_goods_edit(){

		$this->db->Autocommit_start();
		$shopid=intval($this->segment(4));
		$shopinfo=$this->db->GetOne("SELECT * FROM `@#_jf_shoplist` WHERE `id` = '$shopid' and `qishu` order by `qishu` DESC LIMIT 1 for update");
		if($shopinfo['q_end_time'])_message("该商品已经揭晓,不能修改!",G_MODULE_PATH.'/content/jf_goods_list');
		if(!$shopinfo)_message("参数不正确");
		if(isset($_POST['dosubmit'])){
			...
			$goods_key_renqi = isset($_POST['goods_key']['renqi']) ? 1 : 0;
			$is_share = !empty($_POST['is_share']) ? intval($_POST['is_share']) : 0;
			$huocun = !empty($_POST['zongrenshu']) ? intval($_POST['zongrenshu']):100;
            $shenyurenshu = $huocun - $shopinfo['canyurenshu'];
			if(!$cateid)_message("请选择栏目");
			if(!$brandid)_message("请选择品牌");
			if(!$title)_message("标题不能为空");
			if(!$thumb)_message("缩略图不能为空");
			...
			$sql="UPDATE `@#_jf_shoplist` SET `cateid` = '$cateid',
										   `brandid` = '$brandid',
										   `title` = '$title',
										   `title_style` = '$title_style',
										   `title2` = '$title2',
			...
			";
			$s_sid = $shopinfo['sid'];
			$this->db->Query("UPDATE `@#_jf_shoplist` SET `maxqishu` = '$maxqishu' where `sid` = '$s_sid'");
			if($this->db->Query($sql)){
				$this->db->Autocommit_commit();
				_message("修改成功!");
			}else{
		...
		$this->ment=array(
						array("lists","商品管理",ROUTE_M.'/'.ROUTE_C."/jf_goods_list"),
						array("insert","添加商品",ROUTE_M.'/'.ROUTE_C."/jf_goods_add"),
		);
		...
		include $this->tpl(ROUTE_M,'shop.jf_edit');
	}

	//编辑商品
	public function jf_goods_delone(){

		$this->db->Autocommit_start();
		$shopid=intval($this->segment(4));
		$this->db->Query("delete from `@#_jf_shoplist` where `id` = '$shopid'");
		echo 1;die;
	}
	...
	public function goods_add(){
		...
		if(isset($_POST['dosubmit'])){
			...
			$pieces_num = intval($_POST['pieces_num']) ? intval($_POST['pieces_num']) : 1;
			$ispre = intval($_POST['ispre']);

			if($goods_type == 1){
				if(!$pieces)_message("份数错误");
				if(!$pieces_num)_message("购买次数错误");
				if($money % $pieces > 0){
					_message("总价格必须是份数的整数倍");
				}
				$yunjiage = $money / $pieces;
			}
			...
			$query_1 = $this->db->Query("INSERT INTO `@#_shoplist` (`ispre`,`pv`,`yuanjia`,`goods_type`,`pieces_num`,`quyu_begin`,`quyu_end`,`ex_y`,`cateid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `yunjiage`, `zongrenshu`, `canyurenshu`,`shenyurenshu`, `qishu`,`maxqishu`,`thumb`, `picarr`,`content`,`xsjx_time`,`renqi`,`pos`,`bannershop`,`renqipos`,`newpos`,`posthumb`, `time`, `is_share`) VALUES
			                                                       ('$ispre','$pv','$yuanjia','$goods_type','$pieces_num','$quyu_begin','$quyu_end','".htmlspecialchars($_POST['ex_y'])."','$cateid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money', '$yunjiage', '$zongrenshu', '$canyurenshu','$shenyurenshu', '1','$maxqishu', '$thumb', '$picarr', '$content','$xsjx_time','$goods_key_renqi','$goods_key_pos','$goods_key_bannershop','$renqipos','$newpos','$posthumb','$time','$is_share')");
	...
	public function jf_goods_add(){
		...
		if(isset($_POST['dosubmit'])){
			...
			$description = htmlspecialchars($_POST['description']);
			$content = editor_safe_replace(stripslashes($_POST['content']));
			$zongrenshu =!empty($_POST['zongrenshu'])? intval($_POST['zongrenshu']):100;
			$yunjiage = intval($_POST['yunjiage']);
			$promotion_price = intval($_POST['promotion_price']);
			$thumb = htmlspecialchars($_POST['thumb']);
			$maxqishu = intval($_POST['maxqishu']);
			$canyurenshu = 0;
			$goods_key_pos = isset($_POST['goods_key']['pos']) ? 1 : 0;
			...
			if(!$title)_message("标题不能为空");
			if(!$thumb)_message("缩略图不能为空");
            if(!$promotion_price)_message("促销价不能为空");
			...
			$time=time();	//商品添加时间
			$this->db->Autocommit_start();
			$query_1 = $this->db->Query("INSERT INTO `@#_jf_shoplist` (`cateid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`,`promotion_price`, `yunjiage`, `zongrenshu`, `canyurenshu`,`shenyurenshu`, `qishu`,`maxqishu`,`thumb`, `picarr`, `content`,`xsjx_time`,`renqi`,`pos`, `time`, `is_share`) VALUES  ('$cateid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money','$promotion_price', '$yunjiage', '$zongrenshu', '$canyurenshu','$shenyurenshu', '1','$maxqishu', '$thumb', '$picarr', '$content','$xsjx_time','$goods_key_renqi', '$goods_key_pos','$time', '$is_share')");
			$shopid = $this->db->insert_id();
			System::load_app_fun("content");
			if($query_1){
				$this->db->Autocommit_commit();
...
// 		var_dump($categorys);
		...
		if($cateid){
			...
			if(!$cateinfo)_message("参数不正确,没有这个栏目",G_ADMIN_PATH.'/'.ROUTE_C.'/addarticle');
			$categoryshtml.='<option value="'.$cateinfo['cateid'].'" selected="true">'.$cateinfo['name'].'</option>';
			$BrandList=$this->db->GetList("SELECT * FROM `@#_jf_brand` where `cateid`='$cateid'",array("key"=>"id"));
		}else{
			$BrandList=$this->db->GetList("SELECT * FROM `@#_jf_brand` where 1",array("key"=>"id"));
		}
//         var_dump($categoryshtml);
		...
		include $this->tpl(ROUTE_M,'shop.jf_insert');
	...
	public function jf_goods_set(){
		...
		switch($p_key){
			case 'renqi':
				$query = $this->db->Query("UPDATE `@#_jf_shoplist` SET `renqi` = '1' where `id` = '$p_val'");
				break;
		}
	...
	//ajax 删除商品
	public function jf_goods_del(){
		$shopid=intval($this->segment(4));
		$info = $this->db->GetOne("SELECT codes_table FROM `@#_jf_shoplist` WHERE `id` = '$shopid'");
		$table = $info['codes_table'];

		$this->db->Autocommit_start();
		//$q1 = $this->db->Query("INSERT INTO `@#_shoplist_del` select * from `@#_shoplist` where `id` = '$shopid'");
		$q1 = $this->db->Query("DELETE FROM `@#_{$table}` WHERE `s_id` = '$shopid'");
		$q2 = $this->db->Query("DELETE FROM `@#_jf_shoplist` WHERE `id` = '$shopid' LIMIT 1");
		$q3 = $this->db->Query("DELETE FROM `@#_member_go_jf_record` WHERE `shopid` = '$shopid'");
		if($q1 && $q2 && $q3){
			$this->db->Autocommit_commit();
			echo WEB_PATH.'/'.ROUTE_M.'/content/jf_goods_list/';
		}else{
			$this->db->Autocommit_rollback();
			echo "no";
		}
	...
	// 撤销删除
	public function jf_goods_del_key(){
		$shopid=intval($this->segment(4));
		$key=$this->segment(5);
		//撤销
		if($key=='yes'){
			$this->db->Autocommit_start();
			$q1 = $this->db->Query("INSERT INTO `@#_jf_shoplist` select * from `@#_jf_shoplist_del` where `id` = '$shopid' LIMIT 1");
			$q2 = $this->db->Query("DELETE FROM `@#_jf_shoplist_del` WHERE `id` = '$shopid' LIMIT 1");
			if(!$q1 || !$q2){
				$this->db->Autocommit_rollback();
				_message("操作失败");
			}else{
		...
		//从数据库删除
		if($key=='no'){
			$this->db->Query("DELETE FROM `@#_jf_shoplist_del` WHERE `id` = '$shopid' LIMIT 1");
			_message("操作成功");
		}
	}
	...
	//清空回收站
	public function jf_goods_del_all(){
		$this->db->Query("TRUNCATE TABLE  `@#_jf_shoplist_del`");
		_message("清空成功");
	}
	...
	*	商品排序
	*/
	public function jf_goods_listorder(){
		if($this->segment(4)=='dosubmit'){
			foreach($_POST['listorders'] as $id => $listorder){
				$id = intval($id);
				$listorder = intval($listorder);
				$this->db->Query("UPDATE `@#_jf_shoplist` SET `order` = '$listorder' where `id` = '$id'");
			}
			_message("排序更新成功");
		}else{
			_message("请排序");
	...
	*/
	public function goods_max_qishu(){
		...
		if($money < $onemoney){
			echo json_encode(array("meg"=>" 总价不能小于购买价!","err"=>"-1"));
			exit;
		}
	...
	*	重置商品价格
	**/
	public function jf_goods_set_money(){
		...
		$this->db->Autocommit_start();
		$gid = abs(intval($this->segment(4)));
		$shopinfo = $this->db->GetOne("SELECT * FROM `@#_jf_shoplist` where `id` = '$gid' for update");
		if(!$shopinfo || !empty($shopinfo['q_uid'])){
			_message("参数不正确!");exit;
		}

		if(isset($_POST['yunjiage'])){
			$new_one_m = abs(intval($_POST['yunjiage']));
            $promotion_price = abs(intval($_POST['promotion_price']));
			if(!$new_one_m ){
				_message("价格填写错误!");
			}
			if(($new_one_m == $shopinfo['yunjiage'])){
				_message("价格没有改变!");
			}
            if($promotion_price > $new_one_m){
                _message("促销价不能大于商品价格");
            }
			System::load_app_fun("content");
			//$table = $shopinfo['codes_table'];
			$this->db->Autocommit_start();
			$q1 = $this->db->Query("DELETE FROM `@#_member_go_jf_record` WHERE `shopid` = '$gid'");
			$q4 = $this->db->Query("UPDATE `@#_jf_shoplist` SET
			`canyurenshu` = '0',
			`yunjiage` = '$new_one_m',
			`shenyurenshu` = `zongrenshu`,
		...
		include $this->tpl(ROUTE_M,'shop.jf_set_money');
	}
...
//奖品
	public function prize(){
		if(isset($_POST['dosubmit'])){
			...
			for ($i=1; $i < 11; $i++) {
				$t = array();
				if($_POST['num'.$i] && $_POST['money'.$i]){
					$str .= "($i,'".$_POST['num'.$i]."','".$_POST['money'.$i]."','".$open_time."','".$_POST['num'.$i]."'),";
				}
			}
			$str = rtrim($str,',');

			$query1 = $this->db->Query("truncate `@#_prize`");
			$query2 = $this->db->Query("insert into `@#_prize`(`grade`,`num`,`money`,`open_time`,`last`) values $str");
		...
		$list = $this->db->GetList("select * from `@#_prize`");
		if($list){
			$open_time = '';
			$arr = array();
			foreach ($list as $key => $value) {
				$arr[$value['grade']] = $value;
				$open_time = $open_time ? $open_time : $value['open_time'];
			}
			$open_time = date("Y-m-d H:i",$open_time);
		}
		include $this->tpl(ROUTE_M,'prize.index');
