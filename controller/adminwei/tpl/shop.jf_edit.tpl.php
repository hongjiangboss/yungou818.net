...
$(function(){
	...
	if(null!= parentId && ""!=parentId){
		$.getJSON("<?php echo WEB_PATH; ?>/api/brand/json_jf_brand/"+parentId,{cid:parentId},function(myJSON){
		var options="";
		if(myJSON.length>0){
...
<form method="post" action="">
	<table width="100%"  cellspacing="0" cellpadding="0">
    	<tr style="background-color:#FFe;height:50px;display:none;">
			<td align="right" style="width:120px" style="font-weight:bold"></td>
			<td>
            	<a target="_blank" href="<?php echo WEB_PATH;?>/jf_goods/<?php echo $shopinfo['id'];?>"><b>第(<font color="red"><?php echo $shopinfo['qishu']; ?></font>)期  <?php echo $shopinfo['title']; ?></b></a>
				<br />
		...
		<tr>
			...
			<td><input type="text" id="money"  name="zongrenshu" onKeyUp="value=value.replace(/\D/g,'')" value="<?php echo $shopinfo['zongrenshu']?>" style="width:65px; padding-left:0px; text-align:center" class="input-text">
			     <span style="color:red">总库存为：<?php echo $shopinfo['zongrenshu'];?>&nbsp;&nbsp;默认是100</span>
			</td>
		</tr>
        <tr style="display:none;">
			<td align="right" style="width:120px">最大期数：</td>
            <td><input type="text" name="maxqishu" value="1" class="input-text" style="width:50px; text-align:center" onKeyUp="value=value.replace(/\D/g,'')">期,	&nbsp;&nbsp;&nbsp;期数上限为65535期,每期揭晓后会根据此值自动添加新一期商品！</td>
		</tr>
        <tr>
