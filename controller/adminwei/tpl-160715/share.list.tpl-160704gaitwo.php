...
<script type="text/javascript" charset="utf-8" src="<?php echo G_PLUGIN_PATH; ?>/jeDate/jedate.js"></script>
<style>
...
<form action="" method="post" id="form1" style="display:inline-block; ">
	添加时间:
    	  <input name="posttime1" value="<?php echo empty($start_time)?"":date("Y-m-d H:i:s",$start_time);?>" type="text" id="datebut" readonly class="input-text posttime"  readonly="readonly" /><input class="button"  type="button" onclick="getdate('#datebut')" value="选择日期"/>
          &nbsp; <===> &nbsp; 
 		  <input name="posttime2" value="<?php echo empty($end_time)?"":date("Y-m-d H:i:s",$end_time);?>" type="text" id="datebut1" class="input-text posttime"  readonly="readonly"  /><input class="button" onclick="getdate('#datebut1')" type="button" value="选择日期"/>
...
function  getdate($id){
	jeDate({dateCell:$id,isTime:true,format:'YYYY-MM-DD hh:mm:ss'})
}
jeDate({
	//样例参数
