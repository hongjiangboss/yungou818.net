...
$(document).ready(function(){
	...
	CONFIG.model={};
		<?php 
			...
			$goods_list_htmls = $article_list_htmls="'<option value=\"".$cateinfo['info']['template_list']."\" selected>".$cateinfo['info']['template_list']."</option>";
			...
			$goods_show_htmls = $article_show_htmls = "'<option value=\"".$cateinfo['info']['template_show']."\" selected>".$cateinfo['info']['template_show']."</option>";
	...
	?>
	CONFIG.model.id = '<?php echo $cateinfo['model']; ?>';
	CONFIG.model.name = '<?php echo $models[$cateinfo['model']]['name']; ?>';
...
<div name='con-tabv' class="con-tabv">
 ...
 <tbody>
      ...
      <tr>
        <th>栏目名称：</th>
        <td><input type="text" name="info[name]" class="input-text wid140" value="<?php echo $cateinfo['name']; ?>"
         onKeyUp="">
	...
	<tr>
      <th>栏目图片：</th>
        <td>
           <input type="text" id="imagetext" name="thumb" value="<?php echo $cateinfo['info']['thumb']; ?>" class="input-text wid300">
	...
	<tr>
        ...
        <textarea name="info[description]"  maxlength="255" class="overflow" style="width:300px;height:60px;"><?php echo $cateinfo['info']['des']; ?></textarea></td>
    </tr>
</tbody>
</table>
  </div>
	...
	<tbody><tr>
      ...
      <td><input name="setting[meta_title]" type="text" id="meta_title" value="<?php echo $cateinfo['info']['meta_title']; ?>" size="60" maxlength="60" class="input-text"></td>
    </tr>
    <tr>
      <th>META Keywords（栏目关键词）<br>关键字中间用半角逗号,隔开</th>
      <td><textarea name="setting[meta_keywords]" id="meta_keywords" style="width:90%;height:40px"><?php echo $cateinfo['info']['meta_keywords']; ?></textarea></td>
    </tr>
    <tr>
      <th>META Description（栏目描述）<br>针对搜索引擎设置的网页描述</th>
      <td><textarea name="setting[meta_description]" id="meta_description" style="width:90%;height:50px"><?php echo $cateinfo['info']['meta_description']; ?></textarea></td>
    </tr>
</tbody></table>
    </div>
		<!-- 模板选择 -->
...
<form id="form" method="post" enctype="multipart/form-data">
 ...
 <tbody>
      ...
      <tr>
        <th>栏目名称：</th>
        <td><input type="text" name="info[name]" class="input-text" value="<?php echo $cateinfo['name']; ?>"
        onKeyUp="">
	...
	<tr>
   	 	<th style="border-bottom:0px;">描述：</th>
   	 	<td style="border-bottom:0px;"><textarea name="info[description]"  maxlength="255" style="width:300px;height:60px;"><?php echo $cateinfo['info']['des']; ?></textarea></td>
    </tr>
</tbody>
</table>
</div>
<div name='con-tabv' class="con-tabv">
    ...
    <tr>
      ...
      <select name="info[template]">
      	  <option value="<?php echo $cateinfo['info']['template']; ?>" selected><?php echo $cateinfo['info']['template']; ?></option>
           <?php 
    ...
    <div name='con-tabv' class="con-tabv">
		<div style="display:block; position:relative; margin-top:-31px; margin-bottom:-1px; margin-left:-1px;">		
            <script id="myeditor" type="text/plain"><?php echo base64_decode($cateinfo['info']['content']); ?></script>
...
<?php if($catetype=='link'){ ?>
	...
	<tr id="catdir_tr">
      <th>链接地址：</th>
        <td><input type="text" name="info[url]" value="<?php echo $cateinfo['url']; ?>" class="input-text wid300">
