...
$list_where  .= " and (uid>10307 or uid<4) ";
	...
	public function share(){
		...
		$start_time = empty($_POST['posttime1'])?$_GET['start_time']:strtotime($_POST['posttime1']);
		$end_time = empty($_POST['posttime2'])?$_GET['end_time']:strtotime($_POST['posttime2']);
		$is_shaidan = empty($_POST['is_shaidan'])?$_GET['is_shaidan']:$_POST['is_shaidan'];
		$num=20;  
		//判断是否为分页搜索
		if(($_POST['posttime1'] == 0 || $_POST['posttime2'] == 0)&&($start_time == 0||$end_time == 0)){
		    $total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_member_go_record` where auto_user = 1 and `huode` != '0'");
		    $p = ceil($total/$num);
		    $page=System::load_sys_class('page');
		    if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		    $page->config($total,$num,$pagenum,"0",'','','',$is_shaidan);
...
// 		    $_GET['p'] = 1;
		    $total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_member_go_record` where auto_user = 1 and `huode` != '0'  and time<'$end_time' and time>'$start_time'");
		    $p = ceil($total/$num);
		    $page=System::load_sys_class('page');
		    if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		    $page->config($total,$num,$pagenum,"0",'',$start_time,$end_time,$is_shaidan);
		    $recordlist=$this->db->GetPage("SELECT * FROM `@#_member_go_record` where auto_user = 1  and `huode` != '0'  and time<'$end_time' and time>'$start_time'",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));  
		  
		}
	
        $recordlist_shaidan = array();
        $recordlist_meishaidan = array();
	    foreach ($recordlist as $k => $v){
	        $recordlist[$k]['is_share'] = 0;
	       $shaidan=$this->db->GetOne("select * from `@#_member_go_record` where `id`='$v[id]' ");
	        if(!$shaidan){
	            $recordlist[$k]['is_share'] = 1;//不可以晒单
	        }
	        $shaidanyn=$this->db->GetOne("select sd_id from `@#_shaidan` where `sd_shopid`='$shaidan[shopid]' and `sd_userid` = '$shaidan[uid]'");
	        if($shaidanyn){
    		    $recordlist[$k]['is_share'] = 2;//不可重复晒单
    		}
    		$ginfo=$this->db->GetOne("select id,sid,qishu,is_share from `@#_shoplist` where `id`='$shaidan[shopid]' LIMIT 1");
    		if(!$ginfo){
    		    $recordlist[$k]['is_share'] = 3;//商品不能晒单
    		}
    		if($recordlist[$k]['is_share'] == 0){//筛选晒单
    		    $recordlist_meishaidan[] = $v;
    		}else{
    		    $recordlist_shaidan[] = $v;
    		}
	    }
