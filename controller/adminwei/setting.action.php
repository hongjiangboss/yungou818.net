...
class setting extends admin {
	public function __construct(){
		...
		$this->ment=array(
						...
						array("weixin","微信配置",ROUTE_M.'/'.ROUTE_C."/weixin"),
						array("share","分享配置",ROUTE_M.'/'.ROUTE_C."/share"),
	...
	public function sendconfig(){
	  ...
	  include $this->tpl(ROUTE_M,'config.send');
   	...
   	public function weixin(){
			...
			?>
HTML;
			if(!is_writable(G_CONFIG.'weixin.inc.php')) _message('Please chmod winxin  to 0777 !');
		...
		$info=System::load_sys_config("weixin");
		include $this->tpl(ROUTE_M,'config.weixin');
	}

	public function share(){
			if(isset($_POST['dosubmit'])){
			$pc_share_title=htmlspecialchars($_POST['pc_share_title']);
			$share_title=htmlspecialchars($_POST['share_title']);
			$share_desc=htmlspecialchars($_POST['share_desc']);
			$share_pic=htmlspecialchars($_POST['share_pic']);
			$html=<<<HTML
			<?php
			return array (
				'pc_share_title' => '{$pc_share_title}',
			...
			?>
HTML;
			if(!is_writable(G_CONFIG.'share.inc.php')) _message('Please chmod winxin  to 0777 !');
			$ok=file_put_contents(G_CONFIG.'share.inc.php',$html);
			if($ok){
				_message("操作成功");
			}
		}
		...
		include $this->tpl(ROUTE_M,'config.share');
	}
	...
	public function webcfg(){
		if(isset($_POST['dosubmit'])){
			...
			$web_mobile_post=$_POST['web_mobile_post'];
			...
			$this->db->Query("UPDATE `@#_config` SET `value`='$web_mobile_post' WHERE (`name`='web_mobile_post')");
			$ok=$this->cfgPut();
			if($this->db->affected_rows() && $ok){
				_message("修改成功");
			}else{
