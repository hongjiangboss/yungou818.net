...
class user extends admin {
	...
	public function reg(){
		if(isset($_POST['submit-1'])){
			...
			$xitong=isset($_POST['xitong']) ? intval($_POST['xitong']) : 0;
			$yuny=isset($_POST['yuny']) ? intval($_POST['yuny']) : 0;
			$artic=isset($_POST['artic']) ? intval($_POST['artic']) : 0;
			$shop=isset($_POST['shop']) ? intval($_POST['shop']) : 0;
            $exchange=isset($_POST['exchange']) ? intval($_POST['exchange']) : 0;
			$bus=isset($_POST['bus']) ? intval($_POST['bus']) : 0;
			$user=isset($_POST['user']) ? intval($_POST['user']) : 0;
            $tp=isset($_POST['tp']) ? intval($_POST['tp']) : 0;
            $pl=isset($_POST['pl']) ? intval($_POST['pl']) : 0;
             $guanli=isset($_POST['guanli']) ? intval($_POST['guanli']) : 0;
			$password=md5($password2);
			$addtime=time();
			$ip=_get_ip();
			$this->db->Query("INSERT INTO `@#_admin` (`mid`, `username`, `userpass`, `useremail`, `addtime`, `logintime`, `loginip`, `xitong`, `yuny`, `artic`, `shop`,`exchange`,`bus`,`tp`,`pl`,`guanli`) VALUES ('$pmid', '$username', '$password', '$_POST[email]','$addtime','0','$ip', '$xitong','$yuny', '$artic', '$shop', '$exchange', '$bus', '$tp','$pl','$guanli')");
	...
	public function edit(){
		...
		if(isset($_POST['submit-1'])){
			...
			$password=md5($password2);
			$ok = $this->db->Query("UPDATE `@#_admin` SET `userpass`='$password',
                                                          `xitong`='$xitong', 
														  `yuny`='$yuny', 
														  `artic`='$artic', 
														  `shop`='$shop', 
