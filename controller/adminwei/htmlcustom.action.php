...
class htmlcustom extends admin{
	...
	public function lists(){	
		...
		include $this->dwt(DOWN_M,'htmlcustom.lists');
	}
	
	private function AddNewFile(){
		
		if(!file_exists(YYS_CACHES.'caches_codes/tag.sys.package.php')){
				file_put_contents(YYS_CACHES.'caches_codes/tag.package.php','<?php defined(\'IN_YYS\') or exit(\'Access Denied.\'); $TAG = Array(); ?><?php ');
		}
	}
	...
	public function create(){
		...
		if(isset($_POST['submit'])){	
			...
			$this->AddNewFile();
			include  YYS_CACHES.'caches_codes/tag.sys.package.php';
			$header = '<?php defined(\'IN_YYS\') or exit(\'Access Denied.\'); $TAG = Array(); ?><?php '.PHP_EOL;
			$TAG[$tag_name] = $tag_des;
			$con = '$TAG = '.var_export($TAG,true);
			$end = ";".PHP_EOL;			
			$val1 = file_put_contents(YYS_CACHES.'caches_codes/tag.sys.package.php',$header.$con.$end);
			
		
			$str = "QWERTYUIOPLKJHGFDSAZXCVBNM";
			$qz = $str{rand(0,25)}.$str{rand(0,25)}.$str{rand(0,25)}.$str{rand(0,25)};
			$qz.= $str{rand(0,25)}.$str{rand(0,25)}.$str{rand(0,25)}.$str{rand(0,25)};		
			$header  = '<?php defined(\'IN_YYS\') or exit(\'Access Denied.\'); ?>'.PHP_EOL;
			$header .= '<?php '."return <<<".$qz.PHP_EOL;
			$con = $tag_val;
			$end = PHP_EOL.$qz.";".PHP_EOL;			
			$val2 =  file_put_contents(YYS_CACHES.'caches_codes/tag.'.$tag_name.'.php',$header.$con.$end);

			if(gettype($val1)!='integer' || gettype($val2)!='integer'){
				_note("失败");
			}else{
				_note("完成",G_ADMIN_PATH."/htmlcustom/lists");
			}
	...
	public function del(){
		...
		$con = '$TAG = '.var_export($TAG,true);
		$end = ";".PHP_EOL;
		$val = file_put_contents(YYS_CACHES.'caches_codes/tag.sys.package.php',$header.$con.$end);
		if(gettype($val)!='integer'){			
				exit("删除失败");
		}else{
				unlink(YYS_CACHES.'caches_codes/tag.'.$key.'.php');
				echo "yes";
		}		
	}
