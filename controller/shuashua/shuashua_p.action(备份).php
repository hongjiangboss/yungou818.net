...
class shuashua_p extends SystemAction {
	private $db;
	private $categorys;
	private $pay;
	private $autodir = "shuashua";#模块文件名
	...
	public function show(){
		...
		include $this->tpl($this->autodir,'shuashua_p');
	}
	//获取配置文件信息
	public function getxml(){
	   $xml = simplexml_load_file(G_APP_PATH.'controller/'.$this->autodir.'/shuashua_p.xml');
	   return $xml->children();
	}
	/**
	 *更新xml 配置文件信息
...
<timeperiod>'.$xml['timeperiod'].'</timeperiod>
		...
		file_put_contents(G_APP_PATH.'controller/'.$this->autodir.'/shuashua_p.xml',$newxml);
	}
	//功能开启
	...
	public function buyshop($xml,$shopinfo,$member){
		...
		// 	$shopnum = rand(1,3);
		// }
		$shenyu = $shopinfo['zongrenshu'] - $shopinfo['canyurenshu'];
		$max = $shenyu > $shopinfo['zongrenshu']/10 ? $shopinfo['zongrenshu']/10 : $shenyu;
		$shopnum = rand(1,$max);
			...
			if(is_array($member)){
				...
				if($mshop == 1){
					...
					for($i=0;$i<count($shopid);$i++){
						...
						if($shopinfo['quyu_begin'] && $shopinfo['quyu_end']){
							$has_buy = $this->db->GetOne("SELECT count(*) as count FROM `@#_member_go_record` WHERE `shopid` = '$id' and uid >= '{$shopinfo['quyu_begin']}' and uid < '{$shopinfo['quyu_end']}' LIMIT 1");
							if(empty($has_buy['count'])){
								$is_buy = $this->db->GetOne("SELECT count(*) as count FROM `@#_member_go_record` WHERE `shopid` = '$id' LIMIT 1");
								$bid = rand(2,5);
								if(!empty($is_buy['count']) && $bid <=$is_buy['count']){
									$bid_user_id = mt_rand($shopinfo['quyu_begin'],$shopinfo['quyu_end']);
									$member = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid` = '$bid_user_id' LIMIT 1");
								}
							}
						}
	...
	*/
	...
	public function xhaction(){
		...
		while(true){
			...
			if($tp_tmp == 1){
				#调用购买
				$this->g_triggerRequest(WEB_PATH.'/'.$this->autodir.'/shuashua_p/xcaction');
			}else{#此时段不购买
				$t = 1;
			}
			#不购买时只更新配置文件的时间
