...
class shuashua_p extends SystemAction {
	...
	private function CheckAdminInfo($uid=null,$ashell=null){
		...
		if(empty($_SESSION['token']) && $_SESSION['token']!=md5($info['username'].$info['userpass']))return false;
		return true;
	}
	#操作界面显示
...
<timeperiod>'.$xml['timeperiod'].'</timeperiod>
	...
	//购买商品
	private function buyshop($xml,$shopinfo,$member){
		//购买商品数量
		// $shopnum = rand(-100,10);
		// if($shopnum <= -40){
			// $shopnum = 1;
		...
		$min = min($shopinfo['zongrenshu']/10,$shenyu);
		$n = min(10,$min);
        $shopnum = rand(1,$n);
	...
	/*
		...
		返回商品id一维数组
	*/
	private function getshopid($xml,$mshop=1){
		$shopidarray = explode("-",$xml->shopid);#配置文件商品ID    数组
		$shopid = array();
		#生成商品ID数组
		#随机生成购买商品个数  多少个不同商品
	...
	public function xhaction(){
		...
		while(true){
			...
			if($tp_tmp == 1){
				#调用购买
				$this->g_triggerRequest(WEB_PATH.'/'.$this->autodir.'/shuashua_p/xcaction_q');
			}else{#此时段不购买
				$t = 1;
			}
			#不购买时只更新配置文件的时间
