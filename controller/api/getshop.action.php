...
class getshop extends SystemAction {
	...
	public function test(){
		if($this->segment(4) > 0 && $this->segment(4)<500){
			echo json_encode(array("errorCode"=>1));die;
		}
		$t = 1415108344 - time();
		$arr = array("errorCode"=>0,"maxSeconds"=>123,"listItems"=>array(array("id"=>111,"title"=>"test","qishu"=>123,"seconds"=>$t,"thumb"=>"shopimg/20141019/15005683714842.jpg"),array("id"=>123,"title"=>"12411412geg","qishu"=>122,"seconds"=>$t-2122,"thumb"=>"shopimg/20141019/15005683714842.jpg")));
		echo json_encode($arr);die;
	}

	public function getItemInfo(){
		$item_id = $this->segment(4);
		if(empty($item_id)){
			echo json_encode(array("errorCode"=>'1'));
			exit;
		}
		$db = System::load_sys_class('model');
		$info = $db->GetOne("select q_uid,zongrenshu,q_user_code from `@#_shoplist` where `id` ='$item_id' and `q_showtime` = 'Y'");
		if(empty($info)){
			echo json_encode(array("errorCode"=>'1'));
			exit;
		}
	...
	public function getList(){
		$db = System::load_sys_class('model');
		$info = $db->GetList("select qishu,xsjx_time,id,thumb,title,q_uid,q_user,q_end_time from `@#_shoplist` where `q_showtime` = 'Y' order by `q_end_time` ASC");
		if(empty($info)){
			echo json_encode(array("errorCode"=>'1'));
			exit;
		}
		foreach($info as $k=>$v){
			$tmp = substr($v['q_end_time'],0,10)-time();
			$time_arr[] = $tmp;
			$info[$k]['seconds'] = $tmp;
		}
		if($this->segment(4) > 0 && $this->segment(4) < max($time_arr)){
			echo json_encode(array("errorCode"=>'1'));
			exit;
		}
		$arr = array("errorCode"=>0,"maxSeconds"=>max($time_arr),"listItems"=>$info);
		echo json_encode($arr);die;
	}
	...
	//ajax
	public function lottery_shop_json111(){
	...
	//ajax
	public function lottery_shop_huode(){
	...
	public function auto_open_none(){
		$db=System::load_sys_class('model');
		$ginfo = $db->GetList("select * from `@#_shoplist` where `shenyurenshu`=0 and `q_uid` is null limit 10");
		if(!$ginfo){
			echo 1;die;
		}
