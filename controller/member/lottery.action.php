...
class lottery extends SystemAction {
	...
	public function __construct(){
		#                      奖项     说明     红包价格   概率
		$this->prize[] = array('七等奖', '2元红包', 2,  array(800,1000) );
		$this->prize[] = array('六等奖', '5元红包', 5, array(180,1000) );
	...
	public function submit() {
		...
		} else if ( $this->userinfo['LotteryLeave'] <= 0 ) {
		...
		if ( $p == -1 ) {
			...
			$round = $this->round($p);
			$left = $this->userinfo['LotteryLeave'] -1;

			$this->db->Query("UPDATE `@#_member` SET LotteryLeave = LotteryLeave - 1,`money` = `money` + $money where (`uid` = '".$this->userinfo['uid']."')");
