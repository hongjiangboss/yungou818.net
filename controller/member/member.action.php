	...
	*/
	public function lists(){	
		...
		if(!empty($members)){
			...
			if(!empty($uarr)){
				$uarr = implode(",",$uarr);
				$u_band = $this->db->GetList("SELECT * FROM `@#_member_band` WHERE b_uid in($uarr)");
				foreach($members as $k=>$v){
					if($v['band'] == 'weixin'){
						foreach($u_band as $value){
							if($v['uid'] == $value['b_uid']){
								$members[$k]['openid'] = $value['b_code'];
								break;
							}
						}
					}
	...
	public function modify(){
		...
		if(isset($_POST['submit'])){
	        ...
	        $q_begin = intval($_POST['q_begin']);
			$q_end = intval($_POST['q_end']);
			...
			$sql="UPDATE `@#_member` SET `q_begin`='$q_begin',`q_end`='$q_end',`username`='$username',`email`='$email',`mobile`='$mobile',`password`='$password',`money`='$money',`jingyan`='$jingyan',`score`='$score',`emailcode`='$emailcode',`mobilecode`='$mobilecode',`img`='$img',`groupid`='$membergroup',`qianming`='$qianming' WHERE `uid`='$uid'";
			if($this->db->Query($sql)){
				if($q_begin && $q_end){
					$this->db->Query("update `@#_member` set yaoqing='$uid' where uid>=$q_begin and uid<=$q_end and auto_user=1");
				}
				_message("修改成功");
			}else{
				_message("修改失败");
	...
	//会员组
		...
		$num=20;
		$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member`");
		if(empty($wheres)){
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_op_record`"); 
			
		}else{
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_op_record` WHERE $wheres"); 
		...
		$page->config($total,$num,$pagenum,"0");	
		if(empty($wheres)){
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_op_record`  order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		}else{
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_op_record` WHERE  $wheres order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		}
		$members=array();
		...
		include $this->tpl(ROUTE_M,'member.oplist');		
	}
		...
		public function member_fufen(){
			if(isset($_POST['submit'])){			
				...
				$yongjin_on=intval($_POST['yongjin_on']);
				$fufen_yongjin_o=floatval(trim($_POST['fufen_yongjin_o']));
				$fufen_yongjin_w=floatval(trim($_POST['fufen_yongjin_w']));
				$fufen_yongjin_t=floatval(trim($_POST['fufen_yongjin_t']));
	
				$fufen_yongjintx=floatval(trim($_POST['fufen_yongjintx']));
				
				$fufen_qiandao=intval($_POST['fufen_qiandao']);
				$fufen_qiandaoloop=intval($_POST['fufen_qiandaoloop']);
				$fufen_qiandaostart=intval($_POST['fufen_qiandaostart']);
				$fufen_qiandaoend=intval($_POST['fufen_qiandaoend']);
				if($fufen_yuan<=0){
					_message('福分输入有错误');
				}
...
return array (	
	...
	'yongjin_on' => '$yongjin_on',
	'fufen_yongjin_o' => '$fufen_yongjin_o',
	'fufen_yongjin_w' => '$fufen_yongjin_w',
	'fufen_yongjin_t' => '$fufen_yongjin_t',
	'fufen_yongjintx' => '$fufen_yongjintx',
	'fufen_qiandao' => '$fufen_qiandao',
	'fufen_qiandaoloop' => '$fufen_qiandaoloop',
	'fufen_qiandaostart' => '$fufen_qiandaostart',
	'fufen_qiandaoend' => '$fufen_qiandaoend'
);
?>
HTML;
	...
	//佣金提现申请管理
	public function commissions_jf(){

		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_member_cashout1` WHERE 1");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}
		$page->config($total,$num,$pagenum,"0");
		$commissions=$this->db->GetPage("SELECT * FROM `@#_member_cashout1`  WHERE 1",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
        ...
        $fufen = System::load_app_config("user_fufen",'','member');
		include $this->tpl(ROUTE_M,'member.commissions_jf');
	}
	...
	//佣金提现申请审核
	public function commreview_jf(){
	 $id=intval($this->segment(4));


		 $audsta=$this->db->GetOne("SELECT * FROM `@#_member_cashout1` WHERE `id`='$id'  ");
		 $is=$this->db->Query("UPDATE `@#_member_cashout1` SET `auditstatus`='1',shenhe=1,reviewtime=now() where `id`=$id ");
		 if($is==1){
		  //审核通过后将该数据插入到佣金记录表中
		  $type=-3;
		  $content="提现";
	...
	public function recharge(){
		...
		if(isset($_POST['sososubmit'])){
			...
			if(!empty($chongzhi) && $chongzhi!='请选择充值来源'){
				$content=" AND `pay_type` like '%$chongzhi%'";
			}
			$yonghu=isset($_POST['yonghu'])?$_POST['yonghu']:'';
			if(empty($yonghu) || $yonghu=='请选择用户类型'){
				$uid=' AND 1';
		...
		if(empty($wheres)){
			...
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_addmoney_record` WHERE `status`='已付款'");

			$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member_addmoney_record` WHERE `status`='已付款'");

		}else{

			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_addmoney_record` WHERE `status`='已付款' and $wheres");

			$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member_addmoney_record` WHERE `status`='已付款' and $wheres");
		...
		if(empty($wheres)){
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_addmoney_record` where `status`='已付款' order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_addmoney_record` WHERE  `status`='已付款' and $wheres order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}
	...
	public function prize_list(){
		if(isset($_POST['sososubmit'])){
			...
			if($yonghu=='用户手机'){
				if($yonghuzhi){
					$user_uid=$this->db->GetOne("select uid from `#@_member` where `phone_num`='$yonghuzhi'");
					if($user_uid){
						$uid=" AND `uid`='$user_uid[uid]'";
					}else{
						_message($yonghuzhi."用户不存在！");
		...
		$num=20;
		if(empty($wheres)){
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_account` WHERE pay='佣金'"); 
		}else{
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_account` WHERE (pay='佣金') and $wheres"); 
		}
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");	
		if(empty($wheres)){
			$pay_list=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE (pay='佣金') order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		}else{
			$pay_list=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE (pay='佣金') and $wheres order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		}
		$members=array();
		for($i=0;$i<count($pay_list);$i++){
			$uid=$pay_list[$i]['uid'];
			$members[$i]=$this->get_user_name($uid);
		}
		include $this->tpl(ROUTE_M,'member.prize_list');	
	}
