...
class cart extends base {
	...
	public function card_addmoney() {
		...
		$this->db->Query("INSERT INTO `@#_member_addmoney_record`(uid,code,money,pay_type,status,time,score,scookies)VALUES('{$uid}','C".$time."','{$czkdata['mianzhi']}','充值卡','已付款','{$time}',0,0)");
		$out_trade_no = $this->db->insert_id();
		ActivityLotteryPayMoney($out_trade_no, $uid, $czkdata['mianzhi']);
		_message("充值成功!");
	}
	...
	//全额购获取购物车的商品信息到头部
	public function jfcartheader(){
	    
	    $jf_Cartlist = json_decode(stripslashes($this->Cartlist_jf),true);
	    $shopids = '';
	    if(is_array($jf_Cartlist)){
	        foreach($jf_Cartlist as $key => $val){
	            $shopids .= intval($key).',';
	        }
	        $shopids = str_replace(',0', '', $shopids);
	        $shopids = trim($shopids,',');
	    ...
	    $shoplist = array();
	    if($shopids != null){
	       $shoplist = $this->db->GetList("SELECT * FROM `@#_jf_shoplist` where `id` in($shopids) limit 5",array("key"=>"id"));
	    }
	    $MoenyCount = 0;
	    $Cartshopinfo = '{';
	    if(count($shoplist) >= 1){
	        foreach ($jf_Cartlist as $key=>$val){
	            ...
	            if(isset($shoplist[$key])){
	                $shoplist[$key]['cart_gorenci'] = $val['num'] ? $val['num'] : 1;
	                $MoenyCount += $shoplist[$key]['promotion_price']*$shoplist[$key]['cart_gorenci'];
	                $shoplist[$key]['cart_xiaoji'] = substr(sprintf("%.3",$shoplist[$key]['promotion_price']),0,-1);
	                $shoplist[$key]['cart_shenyu'] = $shoplist[$key]['zongrenshu'] - $shoplist[$key]['canyurenshu'];
	                $Cartshopinfo .= "'$key':{'shenyu':".$shoplist[$key]['cart_shenyu'].",'num':".$val['num'].",'money':".$shoplist[$key]['yunjiage']."},";
	            }
	        }
	    ...
	    $MoenyCount = substr(sprintf("%.3f",$MoenyCount),0,-1);
	    $Cartshopinfo .="'MoneyCount':$MoenyCount}";
	    $li="";
	    foreach($shoplist as $st){
	        $li.='<dl class="mycartcur" id="mycartcur'.$st['id'].'">';
	        $li.='<dt class="img"><a href="#"><img src="'.G_UPLOAD_PATH.'/'.$st['thumb'].'"></a></dt>';
	...
	//底部显示ajax显示全额购的商品
	public  function jf_cartshop(){
	    $jfCartlist=json_decode(stripslashes($this->Cartlist_jf),true);
	    $shopids = '';
	    if(is_array($jfCartlist)){
	        foreach($jfCartlist as $key => $val){
	            $shopids .= intval($key).',';
	        }
	        $shopids = str_replace(',0','',$shopids);
	        $shopids = trim($shopids,",");
	    ...
	    $shoplist = array();
	    if($shopids!=null){
	        $shoplist = $this->db->Getlist("SELECT * FROM `@#_jf_shoplist` where `id` in($shopids) limit 7",array("key"=>"id"));
	    }
	    $MoenyCount = 0;
	    $Cartshopinfo = '{';
	    if(count($shoplist) >= 1){
	        foreach ($jfCartlist as $key => $val){
	            ...
	            if(isset($shoplist[$key])){
	                $shoplist[$key]['cart_gorenci'] = $val['num'] ? $val['num'] : 1;
	                $MoenyCount += $shoplist[$key]['promotion_price']*$shoplist[$key]['cart_gorenci'];
	                $shoplist[$key]['cart_xiaoji'] = substr(sprintf("%.3",$shoplist[$key]['yunjiage']*$val[num]), 0,-1);
	                $shoplist[$key]['cart_shenyu'] = $shoplist[$key]['zongrenshu'] - $shoplist[$key]['canyurenshu'];
	                $Cartshopinfo .= "'$key':{'shenyu':".$shoplist[$key]['car_shenyu'].",'num':".$val['num'].",'money':".$shoplist[$key]['yunjiage']."},";
	            }
	        }
	    }
	    $MoenyCount=substr(sprintf("%.3f",$MoenyCount),0,-1);
	    ...
	    foreach ($shoplist as $v){
	        $li .= '<li id="shopid'.$v['id'].'">';
	        $li .= '<a href="javascript:;" onclick="delshop('.$v['id'].')" title="删除" class="Close"></a>';
	        $li .= '<a href="'.WEB_PATH.'/goods/'.$v['id'].'"><img src="'.G_UPLOAD_PATH.'/'.$v['thumb'].'" title="'.$v['title'].'"></a>';
	        $li .= '<span class="orange">￥'.$v['promotion_price'].'</span>';
	        $li .= '</li>';
	        
	    }
	    
        if(count($shoplist)>=7){
            $li .= '<li class="Roll_CartMore"><a target="_blank" title="查看更多" href="'.WEB_PATH.'/member/cart/jf_cartlist">更多<i>&gt;</i></a></li>';
        }
    ...
    //获取全额购购物车里面的商品数量ajax
	public function getjf_number(){
	    $jfCartlist=json_decode(stripslashes($this->Cartlist_jf),true);
        if(!is_array($jfCartlist)){
            echo  0;
        }else{
            echo  count($jfCartlist)-1;
        }
		...
		$Cartshopinfo='{';
			...
			foreach($Cartlist as $key => $val){
					...
					if(isset($shoplist[$key])){
						...
						$shoplist[$key]['cart_xiaoji']=substr(sprintf("%.3f",$shoplist[$key]['yunjiage']*$val['num']),0,-1);
						$shoplist[$key]['cart_shenyu']=$shoplist[$key]['zongrenshu']-$shoplist[$key]['canyurenshu'];
						$Cartshopinfo.="'$key':{'zongrenshu':".$shoplist[$key]['zongrenshu'].",'shenyu':".$shoplist[$key]['cart_shenyu'].",'num':".$val['num'].",'money':".$shoplist[$key]['yunjiage'].",'pieces_num':".$shoplist[$key]['pieces_num']."},";
					}
			}
		}
		...
		if(count($shoplist)>=1){
			foreach($Cartlist as $key => $val){
					...
					if(isset($shoplist[$key])){
						$shoplist[$key]['cart_gorenci']=$val['num'] ? $val['num'] : 1;
						$MoenyCount+=$shoplist[$key]['promotion_price']*$shoplist[$key]['cart_gorenci'];
						$shoplist[$key]['cart_xiaoji']=substr(sprintf("%.3f",$shoplist[$key]['promotion_price']*$val['num']),0,-1);
						$shoplist[$key]['cart_shenyu']=$shoplist[$key]['zongrenshu']-$shoplist[$key]['canyurenshu'];
						$Cartshopinfo.="'$key':{'zongrenshu':".$shoplist[$key]['zongrenshu'].",'shenyu':".$shoplist[$key]['cart_shenyu'].",'num':".$val['num'].",'money':".$shoplist[$key]['yunjiage'].",promotion_price:".$shoplist[$key]['promotion_price']."},";
					}
			}
		}
	...
	public function pay(){
		...
		if(count($shoplist)>=1){
			...
			$shoplist[$key]['cart_gorenci']=$val['num'] ? $val['num'] : 1;
						...
						$shoplist[$key]['cart_xiaoji']=substr(sprintf("%.3f",$shoplist[$key]['yunjiage']*$val['num']),0,-1);
						$shoplist[$key]['cart_shenyu']=$shoplist[$key]['zongrenshu']-$shoplist[$key]['canyurenshu'];
						$ten_per = floor($shoplist[$key]['zongrenshu']) > 0 ? floor($shoplist[$key]['zongrenshu']) : 1;
		...
		$paylist = $this->db->GetList("SELECT * FROM `@#_pay` where `pay_start` = '1'and `pay_class`!='wapalipay'and `pay_class`!='chinabankwap'and `pay_class`!='wxpay_web'and `pay_class`!='unionpay_web'and `pay_class`!='wapjdpay'and `pay_class`!='jubaopaywap'");
	...
	public function jf_paysubmit(){
		...
		if($shop_score && ($jf_use_num <= 0 || $jf_use_num%$fufen['fufen_yuan']!=0)){
			_message("请正确输入福分数量",WEB_PATH.'/member/cart/jf_cartlist');
		}
