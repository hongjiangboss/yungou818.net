...
class index extends base {
	...
	public function init(){
		...
		//最新商品
		$shoplist2=$this->db->GetList("select * from `@#_shoplist` where `q_uid` is null ORDER BY `id` DESC  LIMIT 8");

		//即将揭晓

		$shoplist=$this->db->GetList("select *,`shenyurenshu`/zongrenshu as percent from `@#_shoplist` where `q_uid` is null ORDER BY percent asc LIMIT 8");
		...
		//获取公告 标题
		$ggtitle=$this->db->GetList("select * from `@#_quanzi_tiezi` WHERE qzid=(select id from `@#_quanzi`  where `title` like '%公告%') and `tiezi`='0' and `shenhe` = 'Y' order by zhiding DESC,id DESC LIMIT 3");

        //人气商品
        $shoplistrenqi = $this->db->GetList("select * from `@#_shoplist` where `renqi`='1' and `q_uid` is null ORDER BY id DESC LIMIT 8");
        $MM = $this->db->GetOne("select count(*) as num from `@#_shoplist` where `q_end_time` is not null and `q_showtime` = 'N'");
        $NN = $MM['num'] ? $MM['num'] : 0;
...
//获取广告位商品
		$ggshop=$this->db->GetList("select * from `@#_shoplist` where `bannershop`='1' and `q_uid` is null ORDER BY id DESC LIMIT 2");
		...
		//晒单分享
$shaidan=$this->db->GetList("select * from `@#_shaidan` where grade!='D' order by `sd_id` DESC LIMIT 1");
		$shaidan_two=$this->db->GetList("select * from `@#_shaidan` where grade!='D' order by `sd_id` DESC LIMIT 1,6");
...
public function glist_limit(){
		...
		if($cid && $bid){
			...
			$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `goods_type`=1 and `q_uid` is null and `brandid`='$bid'  and `cateid` in ($sun_id_str)");

		}else{
			if($bid){
				$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `goods_type`=1 and `q_uid` is null and `brandid`='$bid'");
			}elseif($cid){
				$sun_id_str = "'".$cid."'";
				$sun_cate = $this->db->GetList("SELECT cateid from `@#_category` where `parentid` = '$daohang[cateid]'");
				foreach($sun_cate as $v){
				...
				$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `goods_type`=1 and `q_uid` is null and `cateid` in ($sun_id_str)");
			}else{
				$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `goods_type`=1 and `q_uid` is null");
			}
		}
		...
		if($cid && $bid){
			$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `goods_type`=1 and `q_uid` is null and `brandid`='$bid' and `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{
			if($bid){
				$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `goods_type`=1 and `q_uid` is null and `brandid`='$bid' $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}elseif($cid){
				$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `goods_type`=1 and `q_uid` is null and `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}else{
				$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `goods_type`=1 and `q_uid` is null $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}
		}

		$this_time = time();
		include templates("index","glist_limit");
	}
	...
	public function jf_glist(){
	    ...
	    if($cid && $bid){
	        ...
	        $total=$this->db->GetCount("select id from `@#_jf_shoplist` WHERE `q_uid` is null and `brandid`='$bid'  and `cateid` in ($sun_id_str)");
	        ...
	        if($bid){
	            ...
	            $total=$this->db->GetCount("select id from `@#_jf_shoplist` WHERE `q_uid` is null and `cateid` in ($sun_id_str)");
	
	        }else{
	
	            $total=$this->db->GetCount("select id from `@#_jf_shoplist` WHERE `q_uid` is null");
	    ...
	    if($cid && $bid){
	        ...
	        $shoplist=$this->db->GetPage("select * from `@#_jf_shoplist` where `q_uid` is null and `brandid`='$bid' and `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
	
	    }else{
	
	        if($bid){
	
	            $shoplist=$this->db->GetPage("select * from `@#_jf_shoplist` where `q_uid` is null and `brandid`='$bid' $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
	
	        }elseif($cid){
	
	            $shoplist=$this->db->GetPage("select * from `@#_jf_shoplist` where `q_uid` is null and `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
	
	        }else{
	
	            $shoplist=$this->db->GetPage("select * from `@#_jf_shoplist` where `q_uid` is null $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
...
//商品详细
	public function jf_item(){
		...
		$sid_go_record=$mysql_model->GetOne("select * from `@#_member_go_jf_record` where `shopid`='$sid_code[id]' and `uid`='$sid_code[q_uid]' order by `id` DESC LIMIT 1");
		...
		$us2=$mysql_model->GetList("select * from `@#_member_go_jf_record` where `shopid`='".$itemid."' AND `shopqishu`='".$item['qishu']."'ORDER BY id DESC limit 50");


		//期数显示
		$itemlist = $this->db->GetList("select id,qishu,q_uid from `@#_jf_shoplist` where `sid`='$item[sid]' order by `qishu` DESC");

		$loopqishu='<ul class="Period_list">';
		if(!$itemlist[0]['q_uid']){
			if($itemlist[0]['id'] == $item['id'])
				$loopqishu.='<li><a href="'.WEB_PATH.'/jf_goods/'.$itemlist[0]['id'].'"><b class="period_Ongoing period_ArrowCur" style="padding-left:0px;">'."第".$itemlist[0]['qishu']."期<i></i></b></a></li>";
			else
				$loopqishu.='<li><a href="'.WEB_PATH.'/jf_goods/'.$itemlist[0]['id'].'"><b class="period_Ongoing">'."第".$itemlist[0]['qishu']."期<i></i></b></a></li>";
		}else{
			if($itemlist[0]['id'] == $item['id'])
				$loopqishu.='<li><a href="'.WEB_PATH.'/jf_goods/'.$itemlist[0]['id'].'"><b class="period_ArrowCur">'."第".$itemlist[0]['qishu']."期<i></i></b></a></li>";
			else
				$loopqishu.='<li><a href="'.WEB_PATH.'/dataserver/'.$itemlist[0]['id'].'" class="gray02">第'.$itemlist[0]['qishu'].'期</a></li>';
		}
		unset($itemlist[0]);
		...
		include templates("index","jf_item");
	}
