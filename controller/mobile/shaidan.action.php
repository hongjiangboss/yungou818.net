...
class shaidan extends base {
	...
	public function shaidanajax(){
		...
		$count=$this->db->GetList("select * from `@#_shaidan` where grade!='D' order by $sel DESC");
		$shaidan=$this->db->GetList("select * from `@#_shaidan` where grade!='D' order by $sel DESC limit $star,$end");
