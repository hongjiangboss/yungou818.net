...
class mobile extends base {
	...
	public function search_index(){
		$key = mysql_escape_string($_GET['keyword']);
		$shoplist=$this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and title like '%$key%'");
		include templates("mobile/index","search_index");
	}
	...
	public function init(){
		...
		$new_shoplist=$this->db->GetList("select * from `@#_shoplist` where `pos` = '1' and `q_end_time` is null ORDER BY `id` DESC LIMIT 6");
		//即将揭晓
		$shoplist=$this->db->GetList("select * from `@#_shoplist` where `q_end_time` is null ORDER BY `shenyurenshu` ASC LIMIT 6");
		//人气商品
		$shoplistrenqi=$this->db->GetList("select * from `@#_shoplist` where `renqi`='1' and `q_end_time` is null ORDER BY id DESC LIMIT 6");

		$shoplistall=$this->db->GetList("select * from `@#_shoplist` where `q_end_time` is null ORDER BY `zongrenshu` ASC LIMIT 6");
		$max_renqi_qishu = 1;
	...
	public function glist_limit(){
        $webname=$this->_cfg['web_name'];
		$title="限购专区_"._cfg("web_name");
		$key="限购专区";
		include templates("mobile/index","glist_limit");
	}
	//ajax获取商品列表信息
	...
	$end=$_GET['pagesize'] ? intval($_GET['pagesize']) : 100;
		$star=($p-1)*$end;
		...
		if($fen1){
			$count=$this->db->GetList("select * from `@#_shoplist` where goods_type=0 and `q_uid` is null and `cateid` in ($sun_id_str) $select_w");
		}else{
			$count=$this->db->GetList("select * from `@#_shoplist` where goods_type=0 and `q_uid` is null $select_w");
		}
		if($fen1){
			$shoplist=$this->db->GetList("select * from `@#_shoplist` where goods_type=0 and `q_uid` is null and `cateid` in ($sun_id_str) $select_w limit $star,$end");
		}else{
			$shoplist=$this->db->GetList("select * from `@#_shoplist` where goods_type=0 and `q_uid` is null $select_w limit $star,$end");
		}
		$max_renqi_qishu = 1;
		$max_renqi_qishu_id = 1;
...
//商品列表
	public function glist_category(){
		$cate = $this->segment(4);
		$num = $this->segment(5);
		$page = $this->segment(6);
        $webname=$this->_cfg['web_name'];
		$title="商品列表_"._cfg("web_name");
		$key="所有商品";
		include templates("mobile/index","glist_category");
	}
//ajax获取商品列表信息
	public function glistajax_limit(){
	    $webname=$this->_cfg['web_name'];
		$cate_band =htmlspecialchars($this->segment(4));
		$select =htmlspecialchars($this->segment(5));
		$p =htmlspecialchars($this->segment(6)) ? $this->segment(6) :1;
...
$end=$_GET['pagesize'] ? intval($_GET['pagesize']) : 100;
		...
		if($fen1){
			$count=$this->db->GetList("select * from `@#_shoplist` where goods_type=1 and `q_uid` is null and `cateid` in ($sun_id_str) $select_w");
		}else{
			$count=$this->db->GetList("select * from `@#_shoplist` where goods_type=1 and `q_uid` is null $select_w");
		}
		if($fen1){
			$shoplist=$this->db->GetList("select * from `@#_shoplist` where goods_type=1 and `q_uid` is null and `cateid` in ($sun_id_str) $select_w limit $star,$end");
		}else{
			$shoplist=$this->db->GetList("select * from `@#_shoplist` where goods_type=1 and `q_uid` is null $select_w limit $star,$end");
		}
		$max_renqi_qishu = 1;
		$max_renqi_qishu_id = 1;
	...
	public function item(){
		...
		$recordarr = $this->db->GetList("select id,shopid,gonumber,ip,username,uid,time,uphoto from `@#_member_go_record` where `shopid`='$itemid'");
		$wx = 0;
		// if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') != false ) {
            include dirname(__FILE__).DIRECTORY_SEPARATOR."lib".DIRECTORY_SEPARATOR."weixinsdk.php";
            $obj = new JSSDK('wx250cffd3dxxxxf17','61afe26740xxxx08046bf345fd24020');
	...
	public function store() {
        System::load_app_fun('store','member');
        $webname = $this->_cfg['web_name'];
        $key = "排名";
        $fl = $this->segment(4);
        if(!$fl){
        ...
        include templates("mobile/index", "store");
    }
	
	public function buyrecordsAjax() {
        $webname = $this->_cfg['web_name'];
        $key = "所有购买记录";
        $itemid = intval($this->segment(4));
        $orderid = intval($this->segment(5));
        $order = $orderid == 1 ? 'gonumber asc' : 'id desc';
        $cords = $this->db->GetList("select * from `@#_member_go_record` where `shopid`='$itemid' order by $order");
        if (!empty($cords)) {
            foreach ($cords as $k => $v) {
                $cords[$k]['buyTime'] = date("Y-m-d H:i:s", $v['time']);
            }
        }
        echo json_encode(array("Rows" => $cords, "Code" => 0));
    }
	...
	public function goodspost(){
		...
		$id=trim($shop,',');
		if($id){
			$shaidan=$this->db->GetList("select * from `@#_shaidan` where `sd_shopid` IN ($id) and grade!='D' order by `sd_id` DESC");
			$sum=0;
			foreach($shaidan as $sd){
				$shaidan_hueifu=$this->db->GetList("select * from `@#_shaidan_hueifu` where `sdhf_id`='$sd[sd_id]'");
				$sum=$sum+count($shaidan_hueifu);
	...
	public function ajaxbuyrecords(){
	    $webname=$this->_cfg['web_name'];
		$member=$this->userinfo;
		$uid = $member['uid'];
		$cate_band =htmlspecialchars($this->segment(4));
