...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
	...
	public function getUserOrderList(){
	   ...
	   $order=$this->db->GetList("select a.id from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' " );

	   $orderlist['listItems']=$this->db->GetList("select shopid,thumb,qishu,shopname,q_user_code,q_end_time,company,`status`,company_code,q_showtime,shenyurenshu from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.huode!=0 order by a.time desc limit $FIdx,$EIdx " );
		...
		if(empty($orderlist['listItems'])){
		  $orderlist['code']=1;
			...
			foreach($orderlist['listItems'] as $key=>$val){
				...
				$orderlist['listItems'][$key]['company']=$val['company'] ? $val['company'] : '';
				$orderlist['listItems'][$key]['company_code']=$val['company_code'] ? $val['company_code'] : '';
				if($val['q_end_time']!='' && $val['q_showtime']=='N'){
				   //商品已揭晓
					$orderlist['listItems'][$key]['codeState']=3;
					continue;
				}elseif($val['shenyurenshu']==0){
					//商品购买次数已满
					$orderlist['listItems'][$key]['codeState']=2;
					continue;
				}else{
					//进行中
					$orderlist['listItems'][$key]['codeState']=1;
				}
			}
			$orderlist['code']=0;
		}
	...
	public function getUserUnPostList(){
	   ...
	   $member=$this->userinfo;
	    //获得的商品
	    $orderlist=$this->db->GetList("select *,a.id as rid from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.uid='$member[uid]' order by a.time desc" );
		...
		if(empty($unpostlist['listItems'])){
		  ...
		  foreach($unpostlist['listItems'] as $key=>$val){
	        ...
	        foreach($orderlist as $k=>$v){
	        	if($val['id'] == $v['shopid']){
	        		$unpostlist['listItems'][$key]['rid']=$v['rid'];
	        	}
		    }
	      }
		   $unpostlist['code']=0;
