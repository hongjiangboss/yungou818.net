...
include dirname(__FILE__).'/lib/wxpay.class.php';
...
class wxpay_url extends SystemAction {
	...
	public function getResultCode(){
		...
		if($dingdaninfo['status'] == '已付款'){
			...
			echo json_encode(array("code"=>0,"msg"=>"支付成功!"));die;
		}

	}

	public function qiantai(){
		$out_trade_no = $this->out_trade_no ? $this->out_trade_no : $this->segment(4);
	...
	private function weixin_chuli(){
		...
		$up_q1 = $this->db->Query("UPDATE `@#_member_addmoney_record` SET `pay_type` = '微信扫码支付', `status` = '已付款' where `id` = '$dingdaninfo[id]' and `code` = '$dingdaninfo[code]'");
