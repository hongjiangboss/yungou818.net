...
class alipay_url extends SystemAction {
	...
	public function qiantai(){	
		sleep(2);
		file_put_contents("alipay_get.txt",json_encode($_GET));
		...
		if(!$dingdaninfo || $dingdaninfo['status'] == '未付款'){
			_message("支付失败","/?/member/home/userbalance");			
		}else{
			if(empty($dingdaninfo['scookies'])){
				_message("充值成功!","/?/member/home/userbalance");
			}else{
				if($dingdaninfo['scookies'] == '1'){
					_message("支付成功!","/?/member/cart/paysuccess");
				}else{
					_message("商品还未购买,请重新购买商品!","/?/member/cart/cartlist");
				}
	...
	public function houtai(){
		file_put_contents("alipay_post.txt",json_encode($_POST));		
	    include dirname(__FILE__)."/lib/alipay/alipay_notify.class.php";		
		...
		$alipay_config_cacert =  dirname(__FILE__)."/lib/alipay/cacert.pem";	//ca证书路径地址
