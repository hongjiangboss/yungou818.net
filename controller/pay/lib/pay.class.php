...
class pay {
		...
		if(count($shoplist)>=1){
			...
			foreach($Cartlist as $key => $val){
				...
				if(isset($shoplist[$key]) && $shoplist[$key]['shenyurenshu'] != 0){
					...
					if($shoplist[$key]['goods_type'] > 0){
						if($shoplist[$key]['pieces_num'] < $shoplist[$key]['cart_gorenci']){
							return '限购商品数量错误!';
						}
						$go_rec = $this->db->GetOne("select sum(gonumber) as num from `@#_member_go_record` where shopid='{$shoplist[$key]['id']}' and uid='{$this->members['uid']}'");
						$go_rec = $go_rec['num'] ? $go_rec['num'] : 0;
						if($shoplist[$key]['pieces_num'] < $shoplist[$key]['cart_gorenci'] + $go_rec){
							return '限购商品数量错误!';
						}
					}
	...
	private function jf_go_record(){
		...
		if(count($shoplist)>=1){
			...
			foreach($Cartlist as $key => $val){
				$key=intval($key);
				if(isset($shoplist[$key]) && $shoplist[$key]['shenyurenshu'] != 0){
					$shoplist[$key]['cart_gorenci']=$val['num']>0 ? $val['num'] : 1;
					if($shoplist[$key]['cart_gorenci'] >= $shoplist[$key]['shenyurenshu']){
						$shoplist[$key]['cart_gorenci'] = $shoplist[$key]['shenyurenshu'];
					}
					$MoenyCount+=$shoplist[$key]['yunjiage']*$shoplist[$key]['cart_gorenci'];
	...
	private function set_dingdan($pay_type='',$dingdanzhui=''){
	    ...
	    $info = $this->db->GetOne("SELECT * FROM  `@#_member` WHERE (`uid`='$uid') LIMIT 1");
		...
		foreach($this->shoplist as $key=>$shop){
				...
				if($codes_len){
					$insert_html.="('$dingdancode','$dingdancode_tmp','$uid','$username','$uphoto','$shop[id]','$shop[title]','$shop[qishu]','$codes_len','$money','$codes','$pay_type','$ip','$status','$time','$info[auto_user]'),";
				}else{
					if(!$shop['q_uid']){
						$table = '@#_'.$shop['codes_table'];
						$lastnum = $this->db->GetOne("select sum(s_len) as num from $table where `s_id` = '$shop[id]'");
						$lastnum = $lastnum['num'] ? $lastnum['num'] : 0;
						if($lastnum == 0){
							pay_insert_shop($shop);
						}
					}
				}
		}
		$sql="INSERT INTO `@#_member_go_record` (`code`,`code_tmp`,`uid`,`username`,`uphoto`,`shopid`,`shopname`,`shopqishu`,`gonumber`,`moneycount`,`goucode`,`pay_type`,`ip`,`status`,`time`,`auto_user`) VALUES ";
	...
	**/
	...
	private function jf_pay_bag(){
		...
		if($this->fufen){
			$query_4 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '-1', '福分', '购买了商品', '{$this->fufen}', '$time')");
		}
	...
	**/
	...
	private function pay_bag(){
		...
		if(!$this->fufen_to_money){
			...
			$query_add_fufen_1 = $this->db->Query("UPDATE `@#_member` SET `score`= score+'$mygoscore' WHERE (`uid`='$uid')");
