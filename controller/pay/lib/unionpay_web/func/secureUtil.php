...
function sign(&$params) {
	...
	$log->LogInfo ( "Sign key=val&...string >" . $params_str );
	
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	$log->LogInfo ( "Summary sha1x16 >" . $params_sha1x16 );
	// 签名证书路径
	...
	if ($sign_falg) {
		...
		$log->LogInfo ( "Sign string >" . $signature_base64 );
		$params ['signature'] = $signature_base64;
	} else {
		$log->LogInfo ( ">>>>>sign losed<<<<<<<" );
	}
	$log->LogInfo ( '=====sign report end======' );
}
