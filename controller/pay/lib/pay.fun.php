...
function pay_insert_shop($shop='',$type=''){
	...
	$db = System::load_sys_class("model");
	$shop = $db->GetOne("select * from `@#_shoplist` where id='{$shop['id']}'");
	if($shop['q_uid']){
		return true;
	}
	if($shop['xsjx_time'] != '0'){
			...
			if(!empty($ex_code[$ex_rand])){
				...
				*/
				...
				if(!empty($chazhi)){
					...
					$time = $str_t_time + System::load_sys_config('system','goods_end_time');
					$tocode = System::load_app_class("tocode","pay");
					$tocode->shop = $shop;
					$tocode->run_tocode($time,100,$shop['canyurenshu'],$shop);
					$content = addslashes($tocode->go_content);
...
/*
    ...
    dingdancode @订单号
*/
function pay_go_yongjin_bak($uid=null,$dingdancode=null){
...
function pay_go_yongjin($uid=null,$dingdancode=null){
	...
	if(!$config['yongjin_on']){
		return true;
	}
	
	$shoplist = $db->GetList("SELECT `moneycount`,`pv` FROM `@#_shoplist` as a left join `@#_member_go_record` as b on a.id=b.shopid and a.qishu=b.shopqishu WHERE b.`code`='$dingdancode'");
	foreach($shoplist as $v){
		$pv += $v['moneycount'] * $v['pv'];
	}
	if(!$pv){
		return true;
	}
	
    $uinfo=$db->GetOne("SELECT `yaoqing` FROM `@#_member` WHERE `uid`='$uid'");
    if(!$uinfo['yaoqing']){
		return true;
    }
	
	//一级
	$ret = $pv*$config['fufen_yongjin_o'];
	if($uinfo['yaoqing'] >0 && $ret > 0){
		$db->Query("UPDATE `@#_member` SET `yongjin`=yongjin+'$ret' WHERE (`uid`='{$uinfo['yaoqing']}')");
		$db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('{$uinfo['yaoqing']}', '1', '佣金', '会员{$uid}购物订单{$dingdancode}佣金提成', '{$ret}', '$time')");
	}
	
	//二级
	$y_uinfo_t = $db->GetOne("SELECT `yaoqing` FROM `@#_member` WHERE `uid`='{$uinfo['yaoqing']}'");
	$ret_t = $pv*$config['fufen_yongjin_w'];
	if($y_uinfo_t['yaoqing'] >0 && $ret_t > 0){
		$db->Query("UPDATE `@#_member` SET `yongjin`=yongjin+'$ret_t' WHERE (`uid`='{$y_uinfo_t['yaoqing']}')");
		$db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('{$y_uinfo_t['yaoqing']}', '1', '佣金', '会员{$uid}购物订单{$dingdancode}佣金提成', '{$ret_t}', '$time')");
	
		//三级
		$y_uinfo_th = $db->GetOne("SELECT `yaoqing` FROM `@#_member` WHERE `uid`='{$y_uinfo_t['yaoqing']}'");
		$ret_th = $pv*$config['fufen_yongjin_t'];
		if($y_uinfo_th['yaoqing'] >0  && $ret_th > 0){
			$db->Query("UPDATE `@#_member` SET `yongjin`=yongjin+'$ret_th' WHERE (`uid`='{$y_uinfo_th['yaoqing']}')");
			$db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('{$y_uinfo_th['yaoqing']}', '1', '佣金', '会员{$uid}购物订单{$dingdancode}佣金提成', '{$ret_th}', '$time')");
		}
	}
